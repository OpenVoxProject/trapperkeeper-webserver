(ns puppetlabs.trapperkeeper.services.webserver.jetty-service-test
  ...
  (testing "webserver bootstrap throws IllegalArgumentException when neither
    ...
    (is (thrown-with-msg?
          ...
          (tk-log-testutils/with-test-logging
            (with-app-with-empty-config app [jetty-service])))
...
(deftest request-logging-test
  (with-app-with-config
   app
   [jetty-service hello-webservice]
   {:webserver {:port 8080
...
(deftest graceful-shutdown-test
  (testing "jetty webservers shut down gracefully by default"
    (with-app-with-config
      app
      [jetty-service]
      jetty-plaintext-config
